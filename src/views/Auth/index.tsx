...
function SignUp({onPathChange}: AuthComponentProps) {
    ...
    const telAuthCheckResponse = (responseBody: ResponseDto | null) => {
        ...
        const message = 
            !responseBody ? '서버에 문제가 있습니다.' :
            responseBody.code === 'VF' ? '율바른 데이터가 아닙니다.' :
...
function SignIn({onPathChange}: AuthComponentProps) {
    ...
    const signInResponse = (responseBody: SignInResponseDto | ResponseDto | null) => {
        ...
        responseBody.code === 'TCF' ? '서버의 문제가 있습니다.' :
        responseBody.code === 'DBE' ? '서버의 문제가 있습니다.' : '';
    ...
    return(
        ...
        <div className="input-container">
            ...
            <InputBox value={password} onChange={onPasswordChangeHandler} message={message} messageError type='password' label='패스워드' placeholder='비밀번호를 입력해주세요.'/>
        </div>
        <div className="button-container">
...
export default function Auth() {
    ...
    // effect: 첫 로드시에 Query Param의 snsId와 joinPth 존재시 회원가입 화면전환 함수 //
    useEffect(() => {
        if (snsId && joinPath) setPath('회원가입');
    }, []);

    // render: 안증 화면 컴포넌트 렌더링 //
    return (
        <div id="auth-wrapper">
